...
class GitlabApi:
    ...
    API_PATH = "api/v4"
    PROJECT_GROUP_API = API_PATH + "/groups/{}/projects"
    PROJECT_INFO_API = API_PATH + "/projects/{:d}"
    SEARCH_API = API_PATH + "/projects/{}/search"
...
class GitlabRepository(BaseRepository):
    def __init__(self):
        super().__init__(
            GitlabApi.BASE_URL, {"PRIVATE-TOKEN": GitlabConstant.API_TOKEN}
        )
    ...
    def get_project_info(self, project_id):
        status, response = self.__send_get_request(
            GitlabApi.PROJECT_INFO_API.format(project_id)
        )
        if not status:
            return status, response
...
if __name__ == "__main__":
    ...
    if not GitlabConstant.API_TOKEN:
